...
import { Room, RoomEvent, Track, ScreenSharePresets, VideoPreset } from 'livekit-client';
...
export function StreamProvider({ children, initToken, initRole, initName, roomName, wsUrl }) {
  ...
  const handleData = useCallback((payload, participant) => {
    try {
      ...
      switch (msg.type) {
        ...
        case 'file_shared':
          if (msg.file) {
            window.dispatchEvent(new CustomEvent('stream:file_received', { detail: msg.file }));
            // Incrémenter le badge fichiers si le panneau notes n'est pas ouvert
            if (panelRef.current !== 'notes') setUnreadFiles(n => n + 1);
          }
          break;
  ...
  const connect = useCallback(async (token, wsOverride) => {
    ...
    try {
      ...
      // ── E2EE automatique — même séquence que MeetingContext.jsx ─────────
      // Ordre officiel LiveKit : setKey → setE2EEEnabled(true) → connect()
      // On active E2EE pour TOUT LE MONDE par défaut (hôte et spectateurs),
      // sans action manuelle. La clé = roomName (PBKDF2 calculé par le SDK).
      let e2eeReady = false;
      if (isE2EESupported() && keyProviderRef.current) {
        try {
          await keyProviderRef.current.setKey(roomName); // clé = code du live
          await lkRoom.setE2EEEnabled(true);
          e2eeReady = true;
          console.log('[E2EE] ✅ Activé automatiquement — PBKDF2(' + roomName + ') → AES-GCM 256 bits');
        } catch (e2eeErr) {
          console.warn('[E2EE] Activation auto échouée (navigateur limité) :', e2eeErr.message);
        }
      }
      ...
      // Mettre à jour l'état E2EE après connexion réussie
      if (e2eeReady) {
        setE2eeEnabled(true);
        setE2eeMode('global'); // global = actif pour tous par défaut
        console.log('[E2EE] 🌐 E2EE global actif — tous les participants chiffrés');
      }
  ...
  // ── Partage d'écran — qualité HD/4K ──────────────────────────
  // Stratégie qualité :
  //   1. ScreenSharePresets.original → résolution native du bureau (4K si l'écran est 4K)
  //      Le navigateur capture à la résolution de l'écran sans redimensionnement.
  //   2. Fallback h1080fps30 si original échoue (navigateur trop ancien)
  //   3. displaySurface: 'monitor' → capture tout l'écran (pas juste une fenêtre)
  //   4. frameRate 30fps + bitrate 8 Mbps pour la fluidité
  //   5. audio: true → capture le son système (si supporté)
  //
  // ⚠️  La résolution finale dépend de la résolution physique de l'écran partagé.
  //     Si l'écran est 1080p, on aura du 1080p. Si 4K, on aura du 4K.
  //     Le serveur LiveKit peut appliquer une limite côté SFU selon la config.
  const toggleScreenShare = useCallback(async (quality = 'original') => {
    const lk = roomRef.current;
    if (!lk?.localParticipant) return;
    try {
      if (!isScreenSharing) {
        // Options de capture écran HD
        const screenShareOptions = {
          // ScreenSharePresets.original = résolution native, sans resize côté SDK
          // Donne du 4K si l'écran source est 4K
          video: ScreenSharePresets.original,
          // Encodage personnalisé haute qualité
          // maxBitrate 8 Mbps → fluidité sur contenu animé (vidéo, démo code)
          // Pour contenu statique (slides, docs) le SVC réduit automatiquement
          contentHint: 'detail',  // 'motion' pour vidéo, 'detail' pour texte/code
          // Activer le son système si le navigateur le supporte (Chrome 74+)
          audio: true,
        };

        await lk.localParticipant.setScreenShareEnabled(true, screenShareOptions);
      ...
      // Fallback si original échoue (ex : navigateur qui ne supporte pas la résolution native)
      if (!isScreenSharing && e?.message?.includes('constraint')) {
        console.warn('[Stream] screenshare original échoué, fallback h1080fps30');
        try {
          await lk.localParticipant.setScreenShareEnabled(true, {
            video: ScreenSharePresets.h1080fps30,
            audio: true,
          });
          setIsScreenSharing(true);
          const pubs = Array.from(lk.localParticipant.getTrackPublications().values());
      ...
      console.error('[Stream] screenshare:', e);
      setIsScreenSharing(false);
    }
  }, [isScreenSharing]);
  ...
  const setActivePanel = useCallback((panel) => {
    ...
    if (panel === 'notes') setUnreadFiles(0);
  }, []);
  ...
  const value = {
    ...
    viewerCount, reactions, chatMessages, unreadChat, unreadFiles, activePanel,
